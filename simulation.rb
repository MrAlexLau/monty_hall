class Simulation
  attr_reader :result, :num_doors, :interactive_mode, :show_details, :all_doors, :door_with_prize
  ...
  def run
    ...
    prize_found = doors[final_guess - 1][:value] == :car
    ...
    log_result(prize_found, contestant_switched)
  end

  private

  def log_result(prize_found, contestant_switched)
    if prize_found
      # contestant won
